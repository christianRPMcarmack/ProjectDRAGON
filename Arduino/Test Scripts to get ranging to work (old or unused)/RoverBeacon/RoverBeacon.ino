...
void initDW1000() {
  ...
  DW1000.setDefaults();
  DW1000.setDeviceAddress(1);//change this for every device maybe
...
void handleSent() {
    ...
    sentAck = true;
    Serial.println("Signal Sent");//add print for debug
}

void handleReceived() {
    // status change on received success
    receivedAck = true;
    Serial.println("Signal Received");//add print for debug
}
...
//    getAddress(data + 16);
    ...
    noteActivity();
    Serial.print("Rover trying to contact pod ");Serial.println(data[16]);
}
...
//  
  ...
  if (receivedAck) {
    //    Serial.print('R');
    Serial.print("Pod ");Serial.print(data[16]);Serial.println(" received message from rover");
    receivedAck = false;

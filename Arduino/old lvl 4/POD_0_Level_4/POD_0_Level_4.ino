    ...
    if (initialized == true) {
      ...
      if (msgId ==  DATA_REQUEST && data[1] == myNum) { //send next portion of sd card file to rover beacon
        ...
        if (myFile) {
          ...
          dataEnviro[1] = data[2];
          if (sdPointer != 0) {//check the pointer for location in the file
            myFile.seek(sdPointer);
          }
          for (int i = 2; i < LEN_Enviro; i++) {//put data in array for transfer
            if ( myFile.available()) {
        ...
        if (dataEnviro[0] == DATA_COMPLETE) {//if the end of file has been reached
          // Serial.println("Transfer Complete");
          myFile.close();
          sdPointerOld = 0;
          sdPointer = 0;
          SD.remove(file2);//delete file
          myFile = SD.open(file2, FILE_WRITE);//re print the header 
          myFile.print("Time [ms]");
          myFile.print("\t");
          myFile.print("Altitude [m]");
          myFile.print("\t");
        ...
        transmitEnviro();//send data
        noteActivity();
      }


      if (msgId == DATA_REQUEST_LAST_SEND && data[1] == myNum) {//send the previously sent data for error checking
